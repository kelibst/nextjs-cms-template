...
export function PublicationDeleteButton({ id }: { id: string }) {
  const [open, setOpen] = useState(false)
  const router = useRouter()

  const mutation = useMutation({
    mutationFn: () => deletePublication(id),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This action cannot be undone. The publication will be permanently deleted.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
