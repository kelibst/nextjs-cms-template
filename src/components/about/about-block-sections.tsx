...
export function AboutBackground({ body }: AboutBackgroundProps) {
  return (
    <section>
...
interface AboutVisionMissionProps {
  vision?: string
  mission?: string
  body?: string  // backward compat: if vision/mission absent, use body as combined text
}

export function AboutVisionMission({ vision, mission, body }: AboutVisionMissionProps) {
  // If we have separate vision and mission, render two cards
  if (vision || mission) {
    return (
      <section>
  ...
  // Fallback: render body as combined section (backward compat)
  return (
    <section>
      <h2 className="mb-6 text-2xl font-bold text-primary">Vision &amp; Mission</h2>
      <div className="rounded-xl border border-primary-muted bg-primary-subtle p-6">
...
interface AboutObjectivesProps {
  items: string[]
  heading?: string
}

export function AboutObjectives({ items, heading = "Aims & Objectives" }: AboutObjectivesProps) {
  const objectives = items.length > 0 ? items : OBJECTIVES_DEFAULTS

  return (
    <section>
      <h2 className="mb-6 text-2xl font-bold text-primary">{heading}</h2>
...
interface AboutPracticeAreasMiniProps {
  items: { title: string; description: string }[]
  heading?: string
}

export function AboutPracticeAreasMini({ items, heading = "Areas of Practice" }: AboutPracticeAreasMiniProps) {
  const practiceAreas = items.length > 0 ? items : PRACTICE_AREAS_DEFAULTS
...
export function AboutTimeline({ items, heading = "Our History" }: AboutTimelineProps) {
  const timeline = items.length > 0 ? items : TIMELINE_DEFAULTS
